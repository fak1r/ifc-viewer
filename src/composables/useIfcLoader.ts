...
export function useIfcLoader(components: OBC.Components, frags: FragsReadyLike, wasm?: IfcWasmConfig) {
  ...
  async function waitUntil(predicate: () => boolean, timeoutMs = 4000, label = 'waitUntil') {
  ...
  function computeBBoxOfAll(): THREE.Box3 {
    ...
    for (const m of getFragmentsManager().list.values()) box.expandByObject(m.object)
    return box
  }

  async function load(source: ModelSource, opts?: { name?: string; liftBy?: number; autoFit?: boolean }) {
    ...
    try {
      ...
      await waitUntil(() => getFragmentsManager().list.size > 0, 4000, 'firstGroup')
      const frags = getFragmentsManager()
      frags.core.update(true)

      // дождаться непустого bbox
      await waitUntil(() => Number.isFinite(computeBBoxOfAll().min.y), 4000, 'nonEmptyBBox')
  ...
  async function groundToGrid(gridY = 0, extraLift = 0) {
    await waitUntil(() => Number.isFinite(computeBBoxOfAll().min.y), 4000, 'groundToGridBBox')
  ...
  async function alignHorizontally(targetX = 0, targetZ = 0, mode: AlignMode = 'center') {
    await waitUntil(() => Number.isFinite(computeBBoxOfAll().min.y), 4000, 'alignBBox')
    const box = computeBBoxOfAll()

    const refX = mode === 'center' ? (box.min.x + box.max.x) / 2 : mode === 'min' ? box.min.x : box.max.x
    const refZ = mode === 'center' ? (box.min.z + box.max.z) / 2 : mode === 'min' ? box.min.z : box.max.z
