...
export function useMeasurementBase<TMeasurer = any>(deps: MeasurementDeps, params: BaseParams<TMeasurer>) {
  ...
  function ensure(): TMeasurer {
    ...
    const w = world.value
    if (!c || !w) throw new Error('MeasurementBase: components/world are not ready')
    ...
    if (!measurer.value) {
      ...
      if ('unitsList' in m && Array.isArray(m.unitsList)) state.unitsList = [...m.unitsList]
      if ('units' in m) state.units = m.units
      if ('rounding' in m && typeof m.rounding === 'number') state.rounding = m.rounding ?? 2
      ...
      const cGetter =
        params.getColor ??
        ((mm: any) => {
          if (mm?.linesMaterial?.color) return mm.linesMaterial.color as THREE.Color
      ...
      if (params.onItemAdded && m?.list?.onItemAdded?.add) {
        m.list.onItemAdded.add((item: any) => params.onItemAdded!({ item, world: w }))
      }
  ...
  function updateOptions(opts: MeasurementOptions = {}) {
    ...
    if (opts.color !== undefined) {
      const colorStr =
        typeof opts.color === 'number' ? `#${(opts.color as number).toString(16).padStart(6, '0')}` : String(opts.color)
      state.color = colorStr
      const setter =
        params.setColor ??
        ((mm: any, col: THREE.Color) => {
          if (mm?.linesMaterial?.color) (mm.linesMaterial.color as THREE.Color).set(col)
